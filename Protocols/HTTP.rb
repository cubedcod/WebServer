    ...
    def self.call env
      ...
      resource.send(m).yield_self{|status, head, body|           # dispatch request
        ...
        if resource.env[:deny]
          if %w(css otf ttf woff woff2).member?(ext) || %w(activeview ads ad_status.js analytics.js apstag.js attribution beacon.js cast_sender.js css ddljson endscreen.js events FeedQuery id gpt.js gtm.js js load log log_event lvz pay pixel ptracking qoe quant.js query remote.js rundown search seed serviceworker sdk.js session sw.js track tracker view widgets.js).member?(resource.basename) || resource.parts.member?('stats')
            print "🛑" if verbose
