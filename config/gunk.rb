...
class WebResource
  module URIs
    ...
    ScriptGunk = %r([-._\/\(\)\\{}'"\s:?&=~%](6sc|acsbapp?|ad(miral|s|vance)?|affirm|aksb|apple|atrk|b0e8|d([fm]p(xs)?)|en25|fyre|g(aq|eo|igya|t(ag|m))|kr(ux|xd)|licdn|m(ar(insm|keto)|pulse)|nr-data|o(mtrdc|penx)|p(aq|ixel)|(app|grow|king)?sumo|t(aw|rac)k[a-z]*|t(urnto|ynt)|utm|xtlo|zqtk)[-._\/\(\)\\{}'"\s:?&=~%]|
...
pa(ge(s(ense|peed)|.?view)|r(dot|sely|tner)|y[pw]all?)|pbjs|pe(r(imeter.?x|mutive|sonaliz[a-z]*)|tametrics)|pi(ano\.io|co\.tools|n(gdom|img|terest)|wik)|po(rpoiseant|strelease)|pr(e(bid[a-z]+|ssboard)|ivacy-center|ofitwell)|pub(ads|\.network|m(atic|ine))|pu(rechat|sh(bullet|ly|nami))|px-cloud|
