...
class WebResource
  module URIs
    ...
    ScriptGunk = %r([-._\/\(\)\\{}'"\s:?&=~%](6sc|acsbapp?|ad(obe|miral|s|vance)?|affirm|aksb|apple|atrk|b(0e8|eop)|d([fm]p(xs)?)|en25|fyre|g(aq|eo|igya|t(ag|m))|kr(ux|xd)|licdn|m(ar(insm|keto)|pulse)|n(r-data|tv)|o(mtrdc|penx)|p(aq|ixel.*)|(sales)?force|(app|grow|king)?sumo|t(aw|rac)k[a-z]*|t(urnto|ynt)|utm|xtlo|zqtk)[-._\/\(\)\\{}'"\s:?&=~%]|
...
fa(cebook|stclick)|feedbackify|firebase|fo(nt|resee)|freshchat|fullstory|(function|var)[\s\(]+_0x|
...
sa(il.?(horizon|thr[a-z]+)|lesloft\.com)|sc(ene7|(arab|orecard)research)|se(archiq|edtag|gment\.(com|io)|ntry-cdn|rv(edby|ice.?worker)|ssioncam)|sh(arethis|op(ify|per))|si(ftscience|gnup)|slickstream|smart(asset|look)|snowplow|so(cial|nobi|vrn)|spo(nsor[a-z]*|t.?im)|st(atcounter|umbleupon)|swoop\.com|synacor[a-z]*|
