                 ...
                 [{_: :td, class: re.deny? ? 'blocked' :  '', property: k,
                  c: if k == 'uri'                                                                           # main column
                   ...
                   [(resource[Title]||[]).map{|title|
                    ...
                    ([env.has_key?(:proxy_href) ? Webize::HTML.proxy_hrefs(resource[Content], env) : resource[Content], resource[SIOC+'richContent']] unless (resource[Creator]||[]).find{|a|KillFile.member? a.to_s})] # HTML content
                  else
